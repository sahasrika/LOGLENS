...
### Real AI diagnosis with Amazon Bedrock

Install the optional AWS dependencies and configure credentials using the
standard AWS credential chain. LogLens sends only the parsed error records and
sample log lines to Bedrock Runtime Converse; credentials never reach the
frontend.
...
After uploading a `.log` or `.txt` file and selecting **Analyze**, the response
contains the parsed error patterns and a structured `diagnosis` with severity,
root cause, evidence, impact, recommendations, prevention, confidence, and
limitations. If Bedrock is unavailable, parsing still completes and the API
returns an actionable `diagnosis_error`.
...
The `loglens/` core engine will not change. AWS adapters and the Lambda
handler remain outside the core package.

## Lambda packaging

The Lambda entry point is `api.lambda_handler.lambda_handler`. Install the
AWS dependencies with `pip install -e ".[aws]"`. For a reproducible zip,
install the project and dependencies into a clean `package/` directory and
zip that directory together with the source packages. Configure
`LOGLENS_STORAGE_MODE=aws`, `AWS_REGION`, `DYNAMODB_TABLE_NAME`, and
`S3_BUCKET_NAME` in the function environment. Lambda's execution role should
provide only the required S3 object, DynamoDB data, and standard Lambda
logging permissions. No credentials are stored in application configuration.

The project currently targets Python 3.13, which matches the development
environment used here. Select the corresponding AWS Lambda Python runtime
when available, and build native dependencies such as `mmh3` in a compatible
Linux Lambda environment rather than copying Windows wheels.

## Reliability notes

`POST /analyze` accepts an optional user-scoped `Idempotency-Key`. Reusing the
same key for the same user returns the existing analysis. Production rate
limiting should be configured at the API Gateway/WAF layer; an in-memory Lambda
limiter would not be reliable across distributed or recycled execution environments.
